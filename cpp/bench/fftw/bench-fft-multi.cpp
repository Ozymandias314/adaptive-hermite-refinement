...
int main(int argc, const char *argv[]) {
    ...
    std::cout << "Running " << M << " transforms of size " << X << "^2 in batches of " << C << std::endl;
    ...
    for (int m = 0; m < MC; ++m) {
        ...
        for (int c = 0; c < C; ++c) {
            for (int i = 0; i < X2; ++i) {
                int x2 = i % X2, x1 = i / X2;
                in[(m * C + c) * X2 + i] = {
                        std::cos(2.0 * std::numbers::pi * (x1 * x2) / (X2)),
                        std::sin(2.0 * std::numbers::pi * (x1 * x2) / (X2))
                };
            }
        }
        plans[m] = fftw_plan_many_dft(rank, n, howmany,
                                      in.unwrap() + m * C * X2, n, 1, X2,
                                      out.unwrap() + m * C * X2, n, 1, X2,
                                      fftw::BACKWARD, fftw::MEASURE);
    }

    std::cout << "Planning complete!" << std::endl;

    auto [_, wsp] = benchmark(std::cout, [&]() {}, [&]() {
        cilk_for (int m = 0; m < MC; m++) {
            fftw_execute(plans[m]);
        }
    });
