...
!
  ...
  interface scatter
     ...
     module procedure c_redist_44_inv
  end interface
...
!*********************************************************  
  subroutine init_redist(r, char, to_low, to_high, to_list, &
    ...
    type (index_list_type), dimension(0:), intent(in) :: to_list, from_list
    integer, dimension(:), intent (in) :: from_low, to_low, from_high, to_high
    ...
    do ip = 0, nproc - 1
       if (associated(to_list(ip)%first)) then
          ...
          if (associated (to_list(ip)%third)) then !.3
             allocate (r%to(ip)%m(n_to))
             r%to(ip)%m = to_list(ip)%third
          endif

          if (associated (to_list(ip)%fourth)) then !.4
             allocate (r%to(ip)%n(n_to))
             r%to(ip)%n = to_list(ip)%fourth
          endif

          ! account for only effective MPI exchange (leave out intra sub-domain exchange)
    ...
    do ip = 0, nproc - 1
       if (associated(from_list(ip)%first)) then
          ...
          if (associated (from_list(ip)%third)) then !.3
             allocate (r%from(ip)%m(n_from))
             r%from(ip)%m = from_list(ip)%third
          endif

          if (associated (from_list(ip)%fourth)) then !.4
...
!*********************************************************  
  subroutine c_redist_22 (r, from_here, to_here)
    ...
    ! redistribute from local processor to local processor (sub-domain)
    do i = 1, r%from(iproc)%nn
       to_here(r%to(iproc)%k(i), &
               r%to(iproc)%l(i)) &
               = from_here(r%from(iproc)%k(i), &
...
!*********************************************************  
  subroutine c_redist_44 (r, from_here, to_here)
  ...
  end subroutine c_redist_44


!*********************************************************  
  subroutine c_redist_44_inv (r, from_here, to_here)
  ...
  end subroutine c_redist_44_inv
