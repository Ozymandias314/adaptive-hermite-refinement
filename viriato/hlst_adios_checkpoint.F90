...
!>  Fortran module for ADIOS based checkpoint of MPI applications; developed by the EUROfusion High Level Support Team within the HLST-ITM-ADIOS project.
!!
...
!!
!!
!! The user is expected to specify the supported numerical type and number of dimensions at module compile time via three preprocessor symbols, here described:
...
!! It is possible to use tools distributed with ADIOS to manipulate that file; however this should not be necessary, because this module contains all that is needed to read the data back, e.g. in a converter.
!! 
...
!! 
!! When writing (with #hac_write), lower bounds must specified explicitly by the user; the local slice size will be assumed to be the array size unless explicitly provided.
...
!! The upper bounds will be assumed to be the maximal upper bounds across the array slices; the lower bounds will be assumed to be the minimal lower bounds across the array slices.
!!
...
!! 
!! When writing or reading, it is not necessary for all of the array slices to be of the same size, but in no case can a dimension be zero.
!! So, reads (with #hac_read) can be performed on any portion of the global array, as long as indices within its lower and upper bounds are specified.
...
!!     <em> So in case the second I/O is expected to be larger than the first, one should allocate enough at the beginning explicitly</em>.
...
!! \remark As a clean programming practice (to avoid any potential name clash), please don't name any of your identifiers as prefixed with \c 'hac_' or \c 'adios_'.
!! \remark ADIOS-1.5 emits a warning when the local to-save I/O array size exceeds the hard-coded amount of 704643072 bytes.
!!
!! \note
!! \li This module has been used on ADIOS versions 1.5 through 1.8.
!! \li In the current setup, a file (with name specified by the user) and a likely named (with ".dir" suffixed) directory will be created. They shall always be used together, and addressed by the file path.
...
!! \li Handling only of certain simple errors like e.g. 'no file found' is supported; failure to comply with the specifications may go undetected and behaviour undefined.
...
!! \version 20160208
!! 
!! \author Michele Martone, EUROfusion High Level Support Team
!! 
...
! - In order to support multiple types, may declare one array for each type each type DGA,ZGA,SGA,CGA, but keeping only one the 'big' one.
...
#define HLST_HAC_HANDLE_IERR(IERR_,IERR) IF (PRESENT(IERR_)) IERR_ = IERR; IF(IERR==ERRV_N) IERR = OKV_
...
! ############## BEGIN OF PUBLIC DECLARATIONS ##########################
  ...
  PUBLIC :: hac_info, hac_test, hac_stats !< File info / Test / Statistics
  ...
  INTEGER,PARAMETER,PUBLIC :: HAC_OK = 0 !< No error detected
  INTEGER,PARAMETER,PUBLIC :: HAC_ERR_NOFILE = -6 !< File non existent
! ############## END OF PUBLIC DECLARATIONS ############################
...
#endif
  INTEGER,PARAMETER :: OKV_ = HAC_OK !< No error detected
  ...
  INTEGER,PARAMETER :: ERRV_N = HAC_ERR_NOFILE !< File non existent
  ! There are other ERRV_* PUBLIC.
  ...
  LOGICAL,PARAMETER :: RD_ = .TRUE., WR_ = .FALSE. !< For hac_prtsts().
  ...
  INTEGER,PARAMETER :: NDIMSI=NDIMS !< Same, but INTEGER (so, for MPI compatibility)
  ...
  REAL(KIND=TIME_K) :: owt, ort !< Overall write time, overall read time
  INTEGER(KIND=AI_K) :: olrcib, olwcib  ! Overall local read/write contribution in bytes
  INTEGER :: a_buf_mib = 0 ! ADIOS buffer size in MiB (in ADIOS sources adios.c it's multiplied by 1024**2 --- the PDF manual documents it wrong)
...
!
   IF ( amroot ) THEN
    SELECT CASE (ierr)
     ...
     CASE (ERRV_N)
     WRITE(EU,*)BBC,'File does not exist !',ENR
     !CASE (errv_a)
...
!
    ...
    CALL adios_allocate_buffer (a_buf_mib, aerr)
...
!
  LOGICAL FUNCTION hac_file_exists(fname)
   IMPLICIT NONE
!
   CHARACTER(LEN=*),INTENT(IN) :: fname !< Path to the checkpoint file. Any length is allowed.
!
   INQUIRE(file=fname, exist=hac_file_exists)
  END FUNCTION hac_file_exists
!
!> Reads header information from the checkpoint file.
!! Can be called repeatedly to get information out of a checkpoint file.
!! Can be safely used to check for file existence: ierr_==HAC_ERR_NOFILE if checkpoint file not existent.
  SUBROUTINE hac_info(fname, ierr_, GADA_, ntc_, ts_, st_)
   IMPLICIT NONE
!
   INTEGER,INTENT(OUT),OPTIONAL :: GADA_(:) !< Global Array's Dimensions Array.  The user can use a subset of these dimensions to ALLOCATE the right sized (or, SHAPE'd) local array.
   CHARACTER(LEN=*),INTENT(IN) :: fname !< Path to the checkpoint file. Any length is allowed.
   INTEGER,OPTIONAL,INTENT(OUT) :: ierr_ !< Error status variable. If omitted, errors will be fatal.
...
!
   IF ( .NOT. hac_file_exists(fname)) THEN
      ierr = ERRV_N
      GOTO 9999
   END IF
!
...
!> Prints Read/Write Statistics
  SUBROUTINE hac_prtsts(lcib,row,et,fname,tcib_)
   INTEGER(KIND=OFF_K),INTENT(IN) :: lcib ! local contribution in bytes. Can be zero.
...
!! For this reason unlike hac_write, here the index base cannot be unambiguously inferred from the input, and it is advised to specify it.
!!
  SUBROUTINE hac_read(GAS, LBA, fname, LDA_, gaib_, ierr_)
   IMPLICIT NONE
!
   HAC_NTYPE,HAC_DSPEC,INTENT(INOUT) :: GAS !< Global Array Slice. \c SHAPE(GAS) will be assumed to be local slice dimensions; LBOUND(GAS) its global lower bounds, unless \a LBA is provided. Expected to be already allocated.
   INTEGER,INTENT(IN),DIMENSION(:) :: LBA !< Lower Bounds Array. Either 1- or 0-based, on *all* the dimensions. Offset of the local slice in GAS with respect to the global array.
   CHARACTER(LEN=*),INTENT(IN) :: fname !< Path to the checkpoint file. Any length is allowed.
   INTEGER,OPTIONAL,INTENT(IN),DIMENSION(:) :: LDA_ !< Local Dimensions Array. Dimensions of the local slice in GAS. If omitted, SHAPE(GAS) will be used instead.
   INTEGER,OPTIONAL,INTENT(OUT) :: ierr_ !< Error status variable. If omitted, errors will be fatal.
   INTEGER,OPTIONAL,INTENT(IN) :: gaib_ !< Global Array Index Base; can be either 1 or 0; if not specified, this will be taken to be the minimum value across LBA instances.
!
   CHARACTER(LEN=VIL) :: vid
   INTEGER :: i,j
...
!     
   ...
   END IF
!
   CALL hac_prtsts(lcib-mis,RD_,rt,TRIM(fname))
   ort = ort + rt
   olrcib = olrcib + (lcib-mis)
!
9999 HLST_HAC_HANDLE_IERR(ierr_,ierr)
...
!> Writes the global array to the checkpoint file.
  SUBROUTINE hac_write (GAS, LBA, fname, ts_, st_, a_buf_mib_,ws_,ierr_)
   IMPLICIT NONE
!
   HAC_NTYPE,HAC_DSPEC,INTENT(IN) :: GAS !< Global Array Slice. \c SHAPE(GAS) will be assumed to be the local slice dimensions.
   INTEGER,INTENT(IN),DIMENSION(:) :: LBA !< Lower Bounds Array. Either 1- or 0-based, on *all* the dimensions. Offset of the local slice in GAS with respect to the global array.
   CHARACTER(LEN=*),INTENT(IN) :: fname !< Path to the checkpoint file. Any length is allowed.
   INTEGER,OPTIONAL,INTENT(OUT) :: ierr_ !< Error status variable. If omitted, errors will be fatal.
   INTEGER,OPTIONAL,INTENT(IN) :: a_buf_mib_ !< ADIOS buffer size in MiB (1 MiB=1024*1024 bytes; see `man 7 units`). If omitted, it will be automatically set at the first write. If you foresee a subsequent write to be larger, submit an upper limit here at the first call.
...
!
!  Need to document max possible I/O size. Maybe hard-code this. In both ADIOS-1.5 and 1.6,in adios_mpi_amr.c, #define MAX_AGG_BUF 704643072  so max array is half of that: 2*335*1024*1024 = 702545920.
...
!
   ...
   IF ( PRESENT(ws_) ) THEN
    ...
    END IF
   END IF
   CALL hac_prtsts(ags-mis,WR_,wt,TRIM(fname))
   owt = owt + wt
   olwcib = olwcib + (ags-mis)
!
...
!
!> Provide collected statistic values from module hlst_adios_checkpoint.
!! It can be called any amount of times between hac_init and hac_exit.
  SUBROUTINE hac_stats(ierr_,ogwb_,ogrb_)
    IMPLICIT NONE
!
   INTEGER,OPTIONAL,INTENT(OUT) :: ierr_ !< Error status variable. If omitted, errors will be fatal.
   REAL(KIND=8),OPTIONAL,INTENT(OUT) :: ogrb_ !< Overall global read    bytes.
   REAL(KIND=8),OPTIONAL,INTENT(OUT) :: ogwb_ !< Overall global written bytes.
   INTEGER(KIND=AI_K) :: ogwb,ogrb  !< Overall global read/written bytes.
!
...
!
    ierr = OKV_
    IF ( PRESENT(ogwb_) ) THEN
     CALL MPI_Allreduce(olwcib,ogwb,1,HAC_M_INT_T,MPI_SUM,acomm,ierr)
     ogwb_ = REAL(ogwb)
    END IF
    IF ( PRESENT(ogrb_) ) THEN
     CALL MPI_Allreduce(olrcib,ogrb,1,HAC_M_INT_T,MPI_SUM,acomm,ierr)
     ogrb_ = REAL(ogrb)
    END IF
!
...
!
   INTEGER,OPTIONAL,INTENT(OUT) :: ierr_ !< Error status variable. If omitted, errors will be fatal.
!
    CALL hac_prtsts(olwcib,WR_,owt,' all checkpoint files')
    CALL hac_prtsts(olrcib,RD_,ort,' all checkpoint files')
!
...
!
  ...
  END IF
!
  CALL hac_prtsts(INT8(atss*PRODUCT(UB-LB+1)),WR_,awt,(fname),tcib)
  CALL hac_prtsts(INT8(atss*PRODUCT(UB-LB+1)),RD_,art,(fname))
!
  IF ( verbose.GT.erank .AND. amroot ) THEN
  IF (.TRUE.) THEN
...
!
  ...
  CALL MPI_Gather(LBA,NDIMSI,MIT,ALBA,NDIMSI,MIT,RR,ecomm,ierr)
  CALL MPI_Gather(UBA,NDIMSI,MIT,AUBA,NDIMSI,MIT,RR,ecomm,ierr)
  IF (erank.EQ.0) THEN
